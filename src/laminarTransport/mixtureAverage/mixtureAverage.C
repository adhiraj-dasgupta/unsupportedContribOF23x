        ...
        (
           ...
           *pow
            (
                (
                  ...
                  * pow
                    (
                        composition_.W(specieJ)
                       /composition_.W(specieI)
                       ,
                       0.25
                    )    
                ),
           ...
           *X_[specieJ] 
           /sqrt
            (
                dimensionedScalar("one", dimless, 1.0)
              + composition_.W(specieI)/composition_.W(specieJ)  
            )
        );
    }
...
:
    laminarTransport(U, composition, moleFraction_, thermo, mesh),
    Dmix_(n_),
    Vcorr_
    (
        IOobject
    ...
    CutOff_(mixtureAverageDict_.lookupOrDefault("CutOff", 5.0))
{
   ...
   if (thermophoresis_)
   {
       forAll(Y_, specieI)
       {
           if (composition_.W(specieI) <= this->CutOff_)
           {
               dictionary CoeffsDict =
...
) const
{
    ...
    if (thermophoresis_ && (composition_.W(i) <= this->CutOff_))
    {
        phiT_ = 
            linearInterpolate
            (
